...
- **按回合重 roll / 编辑输入**：每个 AI 回合末尾都有 ↻，可从任意回合重写（先确认再执行）；用户消息可 ✎ 编辑，
  保存后从该处重跑。重 roll / 编辑会**回滚该回合改动过的场景与世界状态**；若模型在产出任何内容前失败，
  服务端会把整轮恢复原样，不会白白丢掉一段剧情。
...
- **轻量列表**：侧栏由内存中的摘要缓存提供，不需要为列标题解析每个 story 的完整消息历史。
...
- **工具开关**：被禁用的工具不会出现在 system prompt、也不会出现在请求的 tools 列表里；即使模型
  硬要调用被禁用的工具，agent loop 也会拒绝执行并把错误回喂给它。
- **上下文裁剪**：模型侧 history 同时受 80 条消息上限与约 24000 token 预算约束，从最旧的消息开始裁剪，
  且不会把 tool 结果与其对应的 assistant 工具调用拆散。
